...
export default function SearchBar() {
  ...
  const [results, setResults] = useState<GeographyTopic[]>([]);
  ...
  return (
    <div className="search-container" style={{ position: "relative" }}>
      <input
        ...
        aria-label="Search geography topics"
      ...
      {query.length > 0 && (
        <div className="search-results" style={{ position: "absolute", top: "110%", left: 0, width: "300px", background: "var(--ink)", border: "1px solid var(--line)", zIndex: 20, padding: "10px" }}>
          {results.length > 0 ? (
            results.map((topic) => (
              <Link key={topic.slug} href={`/geography/${topic.slug}`} style={{ display: "block", padding: "5px 0", color: "var(--paper)" }}>
                {topic.title}
              </Link>
            ))
          ) : (
